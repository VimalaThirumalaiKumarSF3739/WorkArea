                    ...
                    for (var i = 0; i < ConnectorLines.Count; i++)
                    {
                        ...
                        else
                        {
                            ...
                            //Checking SegmentSelection behavior and setting SegmentSelectionBrush property. 
                                ...
                                && (series as ISegmentSelectable).SelectionBehavior.SelectionBrush != null
                                )
                            {
                                ConnectorLines[i].Stroke = (series as ISegmentSelectable).SelectionBehavior.SelectionBrush;
                            }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        internal void UpdateSpiderLabels(double pieLeft, double pieRight, Size finalSize, double radius)
        {
            ...
            for (var i = 0; i < orderedAdornments.Count(); i++)
            {
                ...
                if (!Visible) continue;
                var chartAdornmentInfo = this as ChartDataLabelSettings;
        ...
        private void UpdateSelection(DependencyPropertyChangedEventArgs e)
        {
            if (Series != null && Series.ActualArea != null)
            {
                if ((bool)e.NewValue)
                {
                    ...
                    else if (Series is ISegmentSelectable selectable && selectable.SelectionBehavior != null && selectable.SelectionBehavior.SelectedIndex > -1)
                    {
                        Series.UpdateAdornmentSelection(selectable.SelectionBehavior.SelectedIndex);
                    }
                }
                else
                {
                    ...
                    else if ((Series is ISegmentSelectable selectable) && Series.GetEnableSegmentSelection() &&
                        selectable.SelectionBehavior.SelectedIndex > -1
                        && selectable.SelectionBehavior.SelectionBrush != null)
                    {
                        Series.AdornmentPresenter.ResetAdornmentSelection(selectable.SelectionBehavior.SelectedIndex, false);
                    }
                }
            }
        }
