    ...
    public partial class GestureDetector
    {
        ...
        internal void SubscribeNativeGestureEvents(MauiView? mauiView)
        {
            if (mauiView != null)
            {
                ...
                if (nativeView != null)
                {
                    if (pinchGestureListeners?.Count > 0 && nativeView.Context != null)
                    {
                        ...
                        tapGestureListeners?.Count > 0 || doubleTapGestureListeners?.Count > 0)
                    {
                        ...
                        //MAUI-936 IPanGestureListener.OnPan not called when long pressed and moved
                        if (longPressGestureListeners == null || longPressGestureListeners.Count <= 0)
                        {
                            scrollGestureDetector.IsLongpressEnabled = false;
                        }
                    }
        ...
        internal void CreateNativeListener()
        {
            if (MauiView != null)
            {
                ...
                {
                    if (scaleListener == null && pinchGestureListeners?.Count > 0)
                    {
                        scaleListener = new ScaleListener(this);
                        scaleGestureDetector = new ScaleGestureDetector(nativeView.Context, scaleListener);
                    }

                    if (scrollListener == null && (panGestureListeners?.Count > 0 || longPressGestureListeners?.Count > 0 ||
                        tapGestureListeners?.Count > 0 || doubleTapGestureListeners?.Count > 0))
                    {
                        scrollListener = new ScrollListener(this);
                        scrollGestureDetector = new AGestureDetector(nativeView.Context, scrollListener);
        ...
        private void PlatformView_Touch(object? sender, View.TouchEventArgs e)
        {
            ...
            if (motionEvent != null)
            {
                ...
                if (scrollListener != null && motionEvent?.Action == MotionEventActions.Up)
                    scrollListener.EndScrolling(point1);
            }
        }
        ...
        private class ScrollListener : AGestureDetector.SimpleOnGestureListener
        {
            ...
            public override bool OnScroll(MotionEvent? e1, MotionEvent? e2, float distanceX, float distanceY)
            {
                if (e1 != null && e2 != null && e2.PointerCount == 1)
                {
                    ...
                    if (!_isScrolling)
                    {
                        detector?.OnScroll(GestureStatus.Started, point, Point.Zero, Point.Zero);
                        _isScrolling = true;
                    }

                    detector?.OnScroll(GestureStatus.Running, point, new Point(fromPixels(distanceX), fromPixels(distanceY)), Point.Zero);
                }
            ...
            {
                if (e1 != null && e2 != null && e2.PointerCount == 1 && detector != null && detector.panGestureListeners != null && detector.panGestureListeners.Count > 0)
                {
                    Func<double, double> fromPixels = Android.App.Application.Context.FromPixels;
                    this.scrollVelocity = new Point(fromPixels(velocityX), fromPixels(velocityY));
                }
            ...
            /// </summary>
            internal void EndScrolling(Point point)
            {
                detector?.OnScroll(GestureStatus.Completed, point, Point.Zero, this.scrollVelocity);
                _isScrolling = false;
                this.scrollVelocity = Point.Zero;
            }
        }
    }
}
